...
@click.option('--cls_balance', type=float, default=0.001, help='Weight of class balancing component of unsupervised '
                                                              'loss')
@click.option('--cls_balance_loss', type=click.Choice(['bce', 'log', 'bug', '8x', 'uniform_known', '1/20+8/20', 'bug_uniform_known', 'bug_8x']), default='bug',
...
# My added options
@click.option('--use_other_source', type=int, default=0, help='Number of images from class "other" visda2018 source dataset to use (0: None, -1: all)')
        ...
        def compute_aug_loss(stu_out, tea_out):
            ...
            if cls_balance > 0.0:
                ...
                if cls_balance_loss == 'bug_uniform_known':
                    equalise_cls_loss = cls_bal_fn(avg_cls_prob[:-1], float(1.0 / 12))
                elif cls_balance_loss == 'bug_8x':
                    equalise_cls_loss = cls_bal_fn(avg_cls_prob[:-1], float(1.0 / 20)) + cls_bal_fn(avg_cls_prob[-1:], float(8.0 / 20))
                elif cls_balance_loss == 'uniform_known':
                    ...
                    equalise_cls_loss = cls_bal_fn(torch.Tensor((torch.sum(avg_cls_prob[:-2]), avg_cls_prob[-1:])),
                                                   torch.FloatTensor([1/9, 8/9]))
                else:
                    equalise_cls_loss = cls_bal_fn(avg_cls_prob, float(1.0 / n_classes))

                equalise_cls_loss = torch.mean(equalise_cls_loss) * n_classes

                equalise_cls_loss = equalise_cls_loss.cuda() * torch.mean(conf_mask, 0)
            else:
