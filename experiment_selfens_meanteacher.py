...
@click.option('--target_only_cyclegan', is_flag=True, default=False, help='Only use target images generated by augcyclegan')
def experiment(exp, arch, rnd_init, img_size, confidence_thresh, teacher_alpha, unsup_weight,
               ...
               device, num_threads,
               use_other_source, use_other_target, visda2018, n_train_batches, target_only_cyclegan):
    ...
    import image_dataset, visda17_dataset, office_dataset, aug_dataset
    ...
    with torch.cuda.device(device):
        ...
        if exp == 'visda_train_val':
            ...
            d_aug = aug_dataset.Aug_Dataset('/home/samlaf/augcyclegan/data', target_only = target_only_cyclegan)
        ...
        if constrain_crop >= 0:
            def augment(X_sup, y_sup, X_tgt, X_aug):
                X_sup = sup_xf(X_sup)[0]
                X_unsup_both = unsup_xf(X_tgt)[0]
                X_unsup_stu = X_unsup_both[:len(X_tgt)]
                X_unsup_tea = X_unsup_both[len(X_tgt):]
                X_aug_orig = torch.stack([img for img,_ in X_aug])
                X_aug_style = torch.stack([img for _,img in X_aug])
                return X_sup, y_sup, X_unsup_stu, X_unsup_tea, X_aug_orig, X_aug_style
        else:
            def augment(X_sup, y_sup, X_tgt, X_aug):
                X_sup = sup_xf(X_sup)[0]
                X_unsup_stu = unsup_xf(X_tgt)[0]
                X_unsup_tea = unsup_xf(X_tgt)[0]
                return X_sup, y_sup, X_unsup_stu, X_unsup_tea, X_aug
        ...
        def f_train(X_sup, y_sup, X_unsup0, X_unsup1, X_aug_orig, X_aug_style):
            ...
            X_aug_orig = torch.autograd.Variable(X_aug_orig.cuda())
            X_aug_style = torch.autograd.Variable(X_aug_style.cuda())
            ...
            aug_orig_logits_out = student_net(X_aug_orig)
            aug_orig_prob_out = F.softmax(aug_orig_logits_out)
            aug_style_logits_out = teacher_net(X_aug_style)
            aug_style_prob_out = F.softmax(aug_style_logits_out)
            ...
            aug_loss_transfer, _, _ = compute_aug_loss(aug_orig_prob_out, aug_style_prob_out)

            #conf_mask_count = torch.sum(conf_mask)

            #unsup_loss = torch.mean(aug_loss)
            unsup_loss_transfer = torch.mean(aug_loss_transfer)
            loss_expr = clf_loss + unsup_loss_transfer * unsup_weight #+ unsup_loss * unsup_weight
            ...
            outputs = [float(clf_loss.data.cpu()[0]) * n_samples,
                       float(unsup_loss_transfer.data.cpu()[0]) * n_samples,
                       0]
        ...
        aug_ds = data_source.ArrayDataSource([d_aug], repeats=-1)
        train_ds = data_source.CompositeDataSource([sup_ds, tgt_train_ds, aug_ds]).map(augment)
